...
# _ = model.train()
# # export set PYTORCH_CUDA_ALLOC_CONF=max_split_size_mb:32
# #print(torch.cuda.memory_summary())
# model = get_efficient_retinanet(21,"efficientnet-b0", trainable_backbone_layers=0).to(config.device)
# optimizer = torch.optim.Adam(model.parameters(), lr=0.00001)
# _ = model.train()
...
# batch_size = 4
# with tqdm(train_dataloader, unit="" ) as tepoch:
#     loss_records = []
#     for images_ready_in_memory, targets_ready_in_memory in tepoch:
#           tepoch.set_description(f"Train      ep{1}")
#           for idx, (images, targets) in enumerate(zip(split_list(images_ready_in_memory, batch_size), split_list(targets_ready_in_memory, batch_size))):
...
#               print(f"training batch:[{idx * batch_size}]/{256}", end="\r")

#           tepoch.set_postfix(mean_loss = sum(loss_records) / len(loss_records))
#           print("\n")
          

# del loss, losses, reg_loss, cls_loss
# del images, targets
